...
# referencia.
#
# Para generar una sola vez cada ciclo, imponemos el
# criterio lista[1] < lista[-1]. Nótese que, fijado el nodo
# de referencia x_0, las dos orientaciones de un ciclo
#
# x_0 → x_1 → ··· → x_f,
#
# se corresponden con las permutaciones inversas
# (x_0, x_1,...,x_f) y (x_0, x_f,...,x_1).

class Parcial:

    def __init__(self, fich=None, lista=[]):
        ...
        if fich:
            ...
            self.lista = list()
            self.faltan = list()
        self.antecesores = set()

    def es_completa(self):
        return len(self.lista) == self.grafo.order() + 1

    def es_camino_completo(self):
        return len(self.lista) == self.grafo.order()
    ...
    def copia(self):
        ...
        aux.antecesores = self.antecesores.copy()
        return aux
    ...
    def amplía(self):
        ...
        if k == -1:
            ...
            bolsa = self.antecesores\
                    if k == 1 and self.grafo.order() > 2\
                    else self.faltan
            for x in bolsa:
                nuevo = self.copia()
                ...
                if self.longitud() == 1:
                    nuevo.antecesores\
                        = {y for y in nuevo.faltan if y > x}
                else:
                    nuevo.antecesores.discard(x)
                if nuevo.antecesores or k == 1:
                    if k <= 2:
